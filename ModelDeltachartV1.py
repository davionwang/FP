# -*- coding: utf-8 -*-
"""
Created on Thu Nov 24 13:05:43 2022
...
#v1 add Kpler hist/days lines on chart
...
class model_delta_chart():
    ...
    def get_data():
        ...
        supplycountry = DBTOPD('PRD-DB-SQL-211','LNG', 'ana', 'SupplyCountryHist').sql_to_df()
        supplybasin = DBTOPD('PRD-DB-SQL-211','LNG', 'ana', 'SupplyBasinHist').sql_to_df()
        supplyplant = DBTOPD('PRD-DB-SQL-211','LNG', 'ana', 'SupplyPlantHist').sql_to_df()
        
        
        demandcountry = DBTOPD('PRD-DB-SQL-211','LNG', 'ana', 'DemandCountryHist').sql_to_df()
        demandbasin = DBTOPD('PRD-DB-SQL-211','LNG', 'ana', 'DemandBasinHist').sql_to_df()
        
        
        #print(dfdeskmarkethistfull)
        
        return desk_plant_latest, desk_country_latest, desk_supplybasin_latest, desk_supply_hist_plant,desk_supply_hist_country, desk_market_latest, desk_demandbasin_latest, desk_demand_hist, dfdemand, dfsupplyplant, supplycountry, supplybasin, supplyplant,demandcountry, demandbasin
    ...
    def ma30data(supplycountry, supplybasin, supplyplant,demandcountry, demandbasin):
        
        today = datetime.datetime.today()
        supplycountry.set_index('Date', inplace=True)
        supplybasin.set_index('Date', inplace=True)
        supplyplant.set_index('Date', inplace=True)
        
        dfsupply = supplycountry.copy()
        dfsupply[['PB','AB','MENA','Global']] = supplybasin[['PB','AB','MENA','Global']]
        dfsupply = pd.concat([dfsupply, supplyplant], axis=1)
        dfsupply = dfsupply.resample('M').sum().round(1)
        
        dfsupply = dfsupply.loc[str(today.year)+'-01-31':]
        #print(dfsupply)
        #print(dfsupply.columns)
        demandcountry.set_index('Date', inplace=True)
        demandbasin.set_index('Date', inplace=True)
        
        dfdemand = demandcountry.copy()
        dfdemand[['Global','PB','AB','MENA','EoS','WoS','EurDesk','JKTC','MEIP','OtherEur','LatAm','OtherRoW']] = demandbasin[['Global','PB','AB','MENA','EoS','WoS','EurDesk','JKTC','MEIP','OtherEur','LatAm','OtherRoW']]
        dfdemand = dfdemand.resample('M').sum().round(1)
        
        dfdemand = dfdemand.loc[str(today.year)+'-01-31':]
        #print(dfdemand)
        
        
        for i in dfsupply.index:
            if i.month != today.month:
                days=calendar.monthrange(i.year,i.month)
                dfsupply.loc[i]=dfsupply.loc[i]/days[1]
                dfdemand.loc[i]=dfdemand.loc[i]/days[1]
            else:
                dfsupply.loc[i]=dfsupply.loc[i]/today.day
                dfdemand.loc[i]=dfdemand.loc[i]/today.day
        
        dfsupply = dfsupply.round(1)
        dfdemand = dfdemand.round(1)
        #print(dfsupply)
        #print(dfdemand)
        return dfsupply, dfdemand
...
desk_plant_latest, desk_country_latest, desk_supplybasin_latest, desk_supply_hist_plant, desk_supply_hist_country, desk_market_latest, desk_demandbasin_latest, desk_demand_hist, dfdemand, dfsupplyplant, supplycountry, supplybasin, supplyplant,demandcountry, demandbasin = model_delta_chart.get_data()
...
model_delta_chart.ma30data(supplycountry, supplybasin, supplyplant, demandcountry, demandbasin)
'''